...
namespace OnlineSchool.Core.Helper_
{
    public static class GridExtension
    {
        public static GridBoundColumnBuilder<T> ItalianDateFormat<T>(this GridBoundColumnBuilder<T> column)
            where T : class
        {
            return column.ItalianDateFormat(100);
        }
        public static GridBoundColumnBuilder<T> ItalianDateFormat<T>(this GridBoundColumnBuilder<T> column, int width)
            where T : class
        {
            return column.Format("{0:" + ValueFormatToString.DateToDdMMYYY + "}").Width(width);
        }
        ...
        public static GridBoundColumnBuilder<T> HourMinuteFormat<T>(this GridBoundColumnBuilder<T> column)
            where T : class
        {
            return column.Format("{0:" + ValueFormatToString.TimeToHHmm + "}");
        }

        public static GridBoundColumnBuilder<T> DateTimeFormat<T>(this GridBoundColumnBuilder<T> column)
            where T : class
        {
            return column.Format("{0:" + ValueFormatToString.DateToddMMMyyyy + "}");
        }
    }
}
